...
2. Select the option to write a query to specify the data to import. The full dataset contains more than 6.6 million reviews, so we will use SQL queries to only pull in the relevant ones.  
 ...
 3. Under the `Home` tab, click on `Diagram View`. Rearrange your tables so that the `fact_review` table is in the center. Drag the `user_id` from the `dimension_user` table to the `user_id` from the `fact_review` table. Also, drag the `business_id` from the `fact_review` table to the `business_id` in the `dimension_business` table. Save your spreadsheet.

 Now that you have your data model set up, you can start using it to answer questions. Work through as many of the following questions as you can using pivot tables. To build a pivot table spanning multiple tables, click Insert --> PivotTable and choose: `Use this workbook's Data Model`. 
